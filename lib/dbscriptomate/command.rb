  ...
  class Command
    ...
    def initialize(args = [])
      ...
      @opt_parser = OptionParser.new do |opt|
        ...
        opt.on('-t', '--type [TYPE]', String,'is this a DDL or DML script') do |type|
          @parameters[:type] = type
        end

        opt.on('-i', '--initials [INITIALS]', String,'the initials to be used as part of the migration script') do |initials|
          @parameters[:initials] = initials
        end

        opt.on('-d', '--desc [DESC]', String,'the description of what this migration will do') do |desc|
